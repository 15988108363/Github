...
print("*"*20)
print(next(g))
#1.程序开始执行以后，因为foo函数中有yield关键字
# 所以foo函数并不会真的执行
# 而是先得到一个生成器g(相当于一个对象)

#2.直到调用next方法，foo函数正式开始执行
# 先执行foo函数中的print方法，然后进入while循环

#3.程序遇到yield关键字，然后把yield想想成return,return了一个4之后
# 程序停止，并没有执行赋值给res操作
# 此时next(g)语句执行完成
# 所以输出的前两行（第一个是while上面的print的结果
# 第二个是return出的结果）是执行print(next(g))的结果
